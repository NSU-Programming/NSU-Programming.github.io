...
Создадим теперь файл `test.cpp`, содержащий следующий код:
...
Эти два файла составляют синимальный CMake-проект. Вот как выполняется сборка CMake-проекта через консоль: создайте директорию `build`, в которой будет осуществляться сборка и перейдите в неё:

```sh
> mkdir build; cd build
```

Выполните наструйку проекта и запуск сборки:
...
В первой команде мы указали, что сборка будет осуществляться с помощью MinGW и что файлы проекта расположены в директории выше (путь `..`). Вторая команда осуществляет сборку в текущей директории (путь `.`). В директории `build` должен появиться исполняемый файл `test.exe`.

Расширение VS Code для работы с CMake позволяет автоматизировать сборку проекта. Выберите рабочую директорию VS Code (комбинация клавиш `Ctlr+k+o`) с файлами `main.cpp` и `CMakeLists,txt`. Наберите комбинацию клавиш `Ctrl+Shift+P` и в строке сверху наберите команду `>CMake: Configure`. Это запустит настройку инструментов CMake для сборки проекта. После завершения настройки в нижней части окна появятся инструменты управления сборкой:

![](../../figs/textbook-env/cmake-example-project.png)

Нажатие на кнопку "Сборка" запускает сборку, а кнопка &#9655; запускает исполняемый файл.

Если автоматическая настройка CMake привела к ошибке, то, вероятно, инициализация CMake выполнилась без параметра `-G "MinGW Makefiles"`. В этом случае выполните эту команду в консоле, как показано выше. Достаточно выполнить это действие один раз, и конфигурация этого и других проектов будет выполняться верно.

### Работа с git

Покажем как можно работать с git-репозиторием через VS Code. Выполните fork репозитория задания `Hello, Classroom` на GitHub:

![](../../figs/textbook-env/github-fork.png)

Это действие создает новый репозиторий в Вашем аккаунте. Разрешите теперь автоматическое тестирование решение, нажав на большую зеленую кнопку во вкладке Actions:

![](../../figs/textbook-env/github-actions.png)

Для работы с новым репозиторием, его необходимо клонировать на локальную машину. Удобнее всего это делать с помощью протокола ssh. Для этого сначала необходимо [включить OpenSSL Client](http://www.spy-soft.net/windows-10-openssh-client/), который по умолчанию выключен.

Взаимодействие с GitHub-репозиторием будет происходить по протоколу ssh с помощью [техники шифорвания с открытым ключом](https://en.wikipedia.org/wiki/Public-key_cryptography). Создать пару из приватного и публичного ключа можно в консоле:
...
По умолчанию сгенерированные ключи будут расположены в директории `~\.ssh`. Файл с публичным ключом называется `id-rsa.pub`. Публичный ключ нужно добавить на GitHub, для этого откройте раздел `SSH and GPG keys` в меню `Settings`:

![](../../figs/textbook-env/github-ssh-key.png)

и заполните открывшуюся форму. В поле `Key` нужно скопировать содержимое файла `id-rsa.pub`. Проследите, чтобы при копировании не появились лишние переносы строк. Весь ключ должен быть расположен в одной строке.

Теперь мы готовы к клонированию репозитория. Выберите на компьютере директорию, в которой Вы будете работать с заданиями курса и перейдите в неё. Откройте страницу репозитория `hello-classroom` в Вашем аккаунте и скопируйте строку для клонирования через ssh:

![](../../figs/textbook-env/github-clone.png)
...
Строка `git@github.com:fakestud/hello-classroom.git` и есть скопированная строка. Репозиторий был склонирован в директорию `hello-classroom`. Выберите её в качестве рабочей директории VS Code. Прочитайте файл `README.md`, содержащий инструкции по решению задания. После решения задачи и выполните локальную проверку:
...
Редактор VS Code позволяет выполнить эти действия через графический интерфейс. VS Code отслеживает изменения в локальной версии репозитория. Если открыть вкладку контроля версий слева, то вы увидите список изменившихся файлов. В нашем случае это должен быть только `main.cpp`. Выполните команду `git add`, нажав на кнопку `+`:

![](../../figs/textbook-env/vscode-git-add.png)

Затем команду `git commit`, нажав на кнопку &#10003; и введя комментарий в текстовом поле:

![](../../figs/textbook-env/vscode-git-commit.png)

Наконец, выполните команду `git push`:

![](../../figs/textbook-env/vscode-git-push.png)

Если Вы считаете, что задание решено и Вы готовы его сдать, создайте pull request (PR) на GitHub:

![](../../figs/textbook-env/github-pr1.png)
![](../../figs/textbook-env/github-pr2.png)
