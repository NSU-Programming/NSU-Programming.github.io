...
<!-- Если вместо `reshape` использовать [`resize`](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.resize.html), то изменения будут выполнены на месте. Метод `resize` требует больше операций и используется реже, чем `reshape`. -->

<!-- Как и для многих методов класса `np.ndarray`, в `numpy` определены функции-аналоги [`np.reshape`](https://numpy.org/doc/stable/reference/generated/numpy.resize.html) и [`np.resize`](https://numpy.org/doc/stable/reference/generated/numpy.reshape.html), которые имеют тот же смысл, что и методы. -->
...
Помимо равномерного и нормального распределений, модуль `numpy.random` содержит инструменты для получения выборок из множества других дискретных и непрерывных распределений. Полный список доступен в [документации](https://docs.scipy.org/doc//numpy-1.16.0/reference/routines.random.html).
