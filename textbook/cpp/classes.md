...
Сигнатура `LorentzVector&&` обозначает `rvalue` ссылку. Она обозначает временный объект, который можен находится в правой части оператора присваивания, но не может находится в левой. Функцию `std::move` превращает `lvalue`-объект в `rvalue`-объект. Подробнее понятия `lvalue` и `rvalue` объектов мы обсудим в другой части.
