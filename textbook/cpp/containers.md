...
* `find` позволяет найти элемент, если он есть в `set`. Метод возвращает *указатель* на найденный элемент. Если элемент не найден, возвращается специальный указатель, доступный через метод `end()`;
